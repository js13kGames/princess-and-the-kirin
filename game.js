...
function menu(){let state=sim.pause?'pause':sim.state,key=state+sim.index;if(displayState===key)return;displayState=key;document.body.classList.toggle('playing',state==='fight'||state==='defeated');if(state==='fight'||state==='defeated')return;
 ...
 let title='Princess and the Kirin',eyebrow='',description='The Princess and the Kirin seek sunshine and rainbows. Three queens stand in their way.',line='ARROWS move · Hold Z to shoot · SHIFT slows movement and focuses all seven rainbow shots.',button='Begin journey';
